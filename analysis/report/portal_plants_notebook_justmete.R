...
constraint_samples <- apply(plant_abund, MARGIN = 1, FUN = sample_plant_constraints, nsamples = nsamples, fs= F)

save.image('sampling_constraints_justmete.RData')
...
for(i in 1:nrow(plant_abund)) {
  ...
  print(i)
  save.image('getting_stats_justmete.RData')
...
save.image('stats_done_justmete.RData')
...
#if(FALSE) {
write.csv(plant_abund_results, "plants_mete.csv")
save.image('plants_mete.RData')
#}

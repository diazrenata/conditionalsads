...
library(ggplot2)
setwd(here::here())
load(paste0(here::here(), '/plants_done_short.RData'))
...
i = 1
  ...
  r2_plot <- ggplot(data = this_data, aes(x = trmt, y = fs_r2_quantile)) +
    geom_jitter(aes(x = this_data$trmt, y = this_data$fs_r2_quantile, color = this_data$trmt)) +
    ggtitle(paste0(this_season, " r2")) +
    theme_bw()
  
  evar_plot <-  ggplot(data = this_data, aes(x = trmt, y = fs_evar_quantile)) +
    geom_jitter(aes(x = this_data$trmt, y = this_data$fs_evar_quantile, color = this_data$trmt)) +
    ggtitle(paste0(this_season, " evar")) +
    theme_bw()
  
  simp_plot <-  ggplot(data = this_data, aes(x = trmt, y = fs_simp_quantile)) +
    geom_jitter(aes(x = this_data$trmt, y = this_data$fs_simp_quantile, color = this_data$trmt)) +
    ggtitle(paste0(this_season, " Simpson evenness")) +
    theme_bw()
  
  skew_plot <-  ggplot(data = this_data, aes(x = trmt, y = fs_skew_quantile)) +
    geom_jitter(aes(x = this_data$trmt, y = this_data$fs_skew_quantile, color = this_data$trmt)) +
